...
- **GELLO leader:** working as of 2026-09-02. Two FT232H (U2D2) adapters, and the `port:` values already in the configs are correct:
  - `FTAO9WCV` → **left** leader, answers at **57600** with Dynamixel IDs `[8..14]`
  - `FTAO9WPU` → **right** leader, IDs `[1..7]`
  - The shipped configs had these crossed (left leader drove the right follower); fixed 2026-09-08 by swapping the whole `agent` block between the two yamls.
