...
Frames come from a camera-server child process that publishes once per new frame set,
phase-locked to the cameras; the loop runs at an exact 30.0 Hz. Measured: ~0.5 % of
consecutive rows repeat a camera frame. Per-frame `camera_timestamps` in the JSON make
any repeat detectable downstream.
